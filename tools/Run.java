...
public class Run {
    ...
    public static void main(String[] args) throws Exception {
        ...
        switch (mode) {
            ...
            default: System.out.println("usage: java Run.java [server [port] | test | gradle <task..> | mvn <goal..>]"); rc = 2;
        }
        System.exit(rc);
    }
    ...
    static void checkJava() {
        ...
        if (major < MIN_JAVA) {
            System.out.println("!! Java " + major + " is too old. Java " + MIN_JAVA + " or newer: https://adoptium.net");
            System.exit(1);
        }
    ...
    static int plainCompile(String outDir, String... extraSources) throws Exception {
        ...
        System.out.println("[run] compiling -> " + outDir);
        int rc = exec(cmd);
        if (rc != 0) System.out.println("!! COMPILE ERROR -- read the '<File>.java:<line>' line above, fix it, run again.");
        return rc;
    }
    ...
    static int plainServer(String port) throws Exception {
        ...
        System.out.println("[run] starting " + MAIN + " " + port + " (threshold " + threshold + "). Ctrl-C to stop.");
    ...
    static List<Path> listJava(Path dir) throws IOException {
        if (!Files.isDirectory(dir)) throw new FileNotFoundException("no directory " + dir + " -- run this from the project folder");
    ...
    static String wrapperOrPath(String wrapper, String tool) {
        ...
        if (Files.exists(w)) { System.out.println("[run] using wrapper " + w); return w.toAbsolutePath().toString(); }
        String onPath = findOnPath(WINDOWS ? tool + (tool.equals("mvn") ? ".cmd" : ".bat") : tool);
        if (onPath != null) System.out.println("[run] using " + onPath);
        return onPath;
    }
    ...
    static Path ensureTool(String folder, String url, String tool) throws Exception {
        ...
        if (Files.exists(bin)) { System.out.println("[run] using " + bin); return bin; }
        Files.createDirectories(TOOLS_DIR);
        Path zip = TOOLS_DIR.resolve(folder + ".zip");
        System.out.println("[run] no " + tool + " yet -- downloading " + url);
        download(url, zip);
        unzip(zip, TOOLS_DIR);
    ...
    static void unzip(Path zip, Path into) throws IOException {
        try (ZipInputStream z = new ZipInputStream(Files.newInputStream(zip))) {
            for (ZipEntry e; (e = z.getNextEntry()) != null; ) {
                ...
                if (!p.startsWith(into)) throw new IOException("zip entry outside the target folder: " + e.getName());
